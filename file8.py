...
print("Добро пожаловать на игру Угадай Слово.\n\
Вам предстоит ответить на вопрос и если повезет я вам скажу СУПЕР!")
print("Итак, начинаем!\n")

import random
def get_question():
    #открываем файл на чтение
    with open ('l2.txt', 'r', encoding = 'utf-8') as f:
...
# код - угадывание букв
while True:
    user = input ('Введите букву или слово с маленькой буквы через пробел')
    if user == answer:
        print('Вы правильно назвали слово! СУПЕР');break
    if (user in answer):
        print ('Есть такая буква, Но чтобы выиграть этого мало!')
        for i in range(0, len(answer)):
            if answer[i]== user:
                curent_view[i]==user
